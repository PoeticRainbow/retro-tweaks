    ...
    public void retrotweaks$render(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a, Operation<Void> original) {
        ...
        if (!Tweaks.LOADING_SCREEN.get().equals(Versions.MODERN)) {
            ...
            if (!Tweaks.LOADING_SCREEN.get().equals(Versions.INFDEV)) {
                ...
                graphics.fill(RenderPipelines.GUI, 0, 0, width, height, Tweaks.LOADING_SCREEN.get().equals(Versions.ALPHA) ? 0xFF373363 : 0xFFFFFFFF);
                ...
                graphics.blit(RenderPipelines.GUI_TEXTURED, logo, x, y, 0, 0, 128, 128, 128, 128);

                graphics.pose().popMatrix();
            } else {
                graphics.blit(RenderPipelines.GUI_TEXTURED, DIRT_TEXTURE, 0, 0, 0.0F, 0.0F, width, height, 32, 32, 0xFF404040);
                graphics.text(font, loading, 2 + 8, height - 18, 0xFFFFFFFF);
            }
