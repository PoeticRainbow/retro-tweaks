...
public abstract class ClientHooksMixinNeoForge {
    ...
    private static void retrotweaks$remove_neoforge_main_menu(TitleScreen gui, GuiGraphicsExtractor guiGraphics, Font font, int width, int height, int alpha, Operation<Void> original) {
        if (!Tweaks.BETA_MAIN_MENU.get()) {
            original.call(gui, guiGraphics, font, width, height, alpha);
        }
    }
}
